...
RotationBalls.prototype.initTexture = function (htmlCanvas, glContext) {	
	var element = document.getElementById("texture-moon");
	...
	var img = document.getElementById(htmlCanvas);
	ctx.drawImage(img, 0, 0);	

	this.texture = glContext.createTexture();
    handleLoadedTexture(this.texture, canvas, glContext);
}
...
RotationBalls.prototype.initShaders = function (gl, shader) {
	...
	if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) {
		console.log("Could not initialise shaders for rotation balls.");
	}
